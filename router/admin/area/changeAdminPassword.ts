...
export default async function ChangeAdminPassword(req: Request<any>, res: Response<any>) {
	...
	if (req.body.newPassword.trim() == '') {
		...
		log(`[${ip}, !${req.body.area}] bad new admin password`, 'WARNING', __filename);
		return;
	}
	...
	if (req.body.newPassword.length > 512) {
		...
		log(`[${ip}, !${req.body.area}] new password is too long`, 'WARNING', __filename);
		return;
	}
	...
	if (!req.body.allreadyHaseded || req.body.newPassword.length != 128) {
		...
		if (req.body.newPassword != sanitizeString(req.body.newPassword)) {
			...
			log(`[${ip}, !${req.body.area}] new password is not a hash`, 'WARNING', __filename);
			return;
		}
	}
	...
	if (update.matchedCount != 1) {
		...
		log(`[${ip}, !${req.body.area}] no area found from`, 'WARNING', __filename);
		return;
	}

	res.status(200).send({ OK: true, message: 'password of area changed' });
	log(`[${ip}, ${req.body.area}] admin password of area changed from`, 'WARNING', __filename);
}
