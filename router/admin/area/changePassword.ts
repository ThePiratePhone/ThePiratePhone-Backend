...
export default async function ChangePasword(req: Request<any>, res: Response<any>) {
	...
	if (passwordClient != req.body.newPassword.trim()) {
		...
		log(`[${ip}, !${req.body.area}] bad new password (regex faled)`, 'WARNING', __filename);
		return;
	}
	if (passwordClient == '') {
		res.status(400).send({ OK: false, message: 'bad new password' });
		log(`[${ip}, !${req.body.area}] bad new password`, 'WARNING', __filename);
		return;
	}

	if (passwordClient.length > 32) {
		res.status(400).send({ OK: false, message: 'new password is too long (max 32)' });
		log(`[${ip}, !${req.body.area}] new password is too long (max 32)`, 'WARNING', __filename);
		return;
	}
	...
	log(`[${ip}, ${req.body.area}] password of area changed`, 'WARNING', __filename);
}
