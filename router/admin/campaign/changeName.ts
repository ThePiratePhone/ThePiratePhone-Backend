...
export default async function changeName(req: Request<any>, res: Response<any>) {
	...
	if (!campaign) {
		...
		log(`Wrong campaign id from ${area.name} (${ip})`, 'WARNING', 'changeName.ts');
		return;
	}

	if (req.body.newName.trim() == '') {
		res.status(401).send({ message: 'Wrong name id', OK: false });
		log(`Wrong name from ${area.name} (${ip})`, 'WARNING', 'changeName.ts');
		return;
	}
	...
	if (output.matchedCount != 1) {
		...
		log(`Campaign not found from ${ip}`, 'WARNING', 'changeName.ts');
		return;
	}

	res.status(200).send({ message: 'OK', OK: true });
	log(`Campaign name changed from ${ip} (${area.name})`, 'INFORMATION', 'changeName.ts');
}
